...
export function buildSubscriptionPlans(currency: BillingCurrency): SubscriptionPlan[] {
  const plusAnnual = getAnnualPrice('plus', currency);
  const proAnnual = getAnnualPrice('pro', currency);
  ...
  return [
    ...
    },
    {
      ...
      annualPrice: currency === 'INR' ? PLUS_ANNUAL_INR : PLUS_ANNUAL_USD,
      currency,
      priceDisplay: `${formatPrice(plusAnnual, currency)} / year`,
      features: [
    ...
    },
    {
      ...
      annualPrice: currency === 'INR' ? PRO_ANNUAL_INR : PRO_ANNUAL_USD,
      currency,
      priceDisplay: `${formatPrice(proAnnual, currency)} / year`,
      highlighted: true,
      features: [
...
/** @deprecated Use buildSubscriptionPlans(currency) */
export const PLANS: SubscriptionPlan[] = buildSubscriptionPlans('INR');
